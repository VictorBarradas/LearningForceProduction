    ...
    methods
        function obj = nnetworkBP(nUnits)
            ...
            for i = 1:obj.nLayers - 1
                obj.W{i} = normrnd(0,0.5,nUnits(i)+1,nUnits(i+1));
        ...
        function neural_output = network_feedforward(obj,input)
            ...
            obj.layerInput{1} = [input;ones(1,size(input,2))];
            %obj.layerInput{1} = [input;1];
            %obj.layerInput{1} = [input];
            for i = 1:obj.nLayers - 1
                obj.layerOutput{i} = obj.W{i}'*obj.layerInput{i};
                obj.layerOutput{i} = 1./(1 + exp(-obj.layerOutput{i}));
                if i < obj.nLayers - 1
                    obj.layerInput{i+1} = [obj.layerOutput{i};ones(1,size(input,2))];
                    %obj.layerInput{i+1} = [obj.layerOutput{i};1];
